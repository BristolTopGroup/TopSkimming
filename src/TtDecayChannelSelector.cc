...
TtDecayChannelSelector::TtDecayChannelSelector(const edm::ParameterSet& cfg) :
		invert_(cfg.getParameter<bool>("invert")), allowElectron_(false), allowMuon_(false), allow1Prong_(false), allow3Prong_(
				false) {
...
bool TtDecayChannelSelector::operator()(const reco::GenParticleCollection& parts, std::string inputType) const {
	...
	if (verbose) {
		...
		std::cout << "----------------------" << "\n";
		std::cout << " iTop    : " << iTop << "\n";
		std::cout << " iBeauty : " << iBeauty << "\n";
		std::cout << " iElec   : " << iElec << "\n";
		std::cout << " iMuon   : " << iMuon << "\n";
		std::cout << " iTau    : " << iTau + iLep;
...
bool TtDecayChannelSelector::search(reco::GenParticleCollection::const_iterator& part, int pdgId,
	...
	if (inputType == kGenParticles) {
		...
		//the correct top quark is status 2 and has at least 2 daughters
		return (std::abs(part->pdgId()) == pdgId && part->status() == TopDecayID::stable
				 && !containts_itself) ? true : false;
	} else {
